...
[Strong-Weak Coupling Duality in Scalar Lattice QFT with Application to Strong Coupling Expansions](https://arxiv.org/abs/2207.11503).
...
- **Two-point function:** HMC evaluation of \(D(p)=\langle\phi(p)\phi(-p)\rangle\) along the lattice-momentum slice \(p=(2\pi k/M,0,\ldots,0)\).
...
Run the complete \(d=2\) numerical pipeline:

```bash
python3 run_pipeline.py free-energy --config configs/free_energy_d2.json --overwrite
```

For \(d=3\), replace `free_energy_d2.json` with `free_energy_d3.json`.

This command performs two steps:

1. `hmc_free_energy.py` runs the HMC simulations and writes the measured \(\langle\phi^4\rangle\) values and their blocked-jackknife uncertainties.
2. `files_to_free_energy_num.py` constructs a cubic spline through these values, integrates it over \(g^4\), and propagates the HMC statistical uncertainties to the free energy.

To generate the comparison and error plots as well:

```bash
python3 run_pipeline.py free-energy --config configs/free_energy_d2.json --with-plots
```
...
Run the \(d=2\) HMC calculation:

```bash
python3 run_pipeline.py two-point --config configs/two_point_d2.json --overwrite
```

For \(d=3\), replace `two_point_d2.json` with `two_point_d3.json`.

Generate a strong-coupling comparison plot:

```bash
python3 run_pipeline.py two-point --config configs/two_point_d2.json --skip-simulation --with-plots --regime strong
```

For the weak-coupling comparison, replace `strong` with `weak`.

The two-point pipeline does not store complete field configurations. Each retained configuration is immediately reduced to its momentum-space correlator estimator, and only the resulting per-configuration correlator values are kept in memory for statistical analysis.
...
The available production and smoke-test configurations are stored in `configs/`:
...
The \(d=2\) and \(d=3\) production configurations reproduce the numerical calculations discussed in the article. The smoke-test configurations use substantially smaller lattices and shorter Markov chains and are intended only to verify that the installation and complete pipeline work correctly. An additional \(d=4\) free-energy configuration is also provided.

All relative paths are resolved from the repository root.

### Lattice parameters

For example, `configs/free_energy_d2.json` uses
...
For the free-energy production runs, the HMC grid is specified by `couplings_g4`, for example
...
For the free-energy calculation, these values also form the interpolation grid used for thermodynamic integration. A denser grid can reduce interpolation error in regions where \(\langle\phi^4\rangle\) varies rapidly, at the cost of additional HMC simulations.
...
The approximate number of retained measurements per parameter combination is
...
The current HMC pipelines estimate statistical errors with a blocked jackknife over consecutive retained measurements. The default block size used in the implementation is
\(\max(1,\lfloor N_{\mathrm{samples}}/100\rfloor)\).
...
The current implementation uses cubic-spline interpolation of the simulated \(\langle\phi^4\rangle\) values. The spline is integrated through its analytic piecewise-polynomial antiderivative, so the thermodynamic-integration step does not use adaptive numerical quadrature.

The uncertainty of the free energy is propagated from the blocked-jackknife uncertainties of \(\langle\phi^4\rangle\). Since spline interpolation followed by integration is linear in the sampled values, the code computes the corresponding integration weights and, assuming statistically independent HMC runs at different coupling values, combines the variances in quadrature.
...
- `free_energy`: CSV file containing the free energy obtained by thermodynamic integration and the propagated HMC statistical uncertainty in `f_error`. The legacy `quadrature_error` column is currently retained for compatibility and is zero for the cubic-spline antiderivative used by the current implementation.
...
For example, the \(d=2\) free-energy configuration contains
...
- `core/` contains the HMC lattice implementation and correlation/error-estimation utilities.
...
The numerical estimator evaluates the one-dimensional momentum slice
\(p=(2\pi k/M,0,\ldots,0)\), \(k=0,\ldots,M-1\). For each retained field configuration, `core/utils.py` sums over the transverse lattice directions, performs a one-dimensional FFT, and stores \(|\phi_p|^2/N\). `hmc_two_point.py` then applies the blocked jackknife independently at each momentum.
...
Therefore, writing \(\lambda=g^4\),

\[
f_M(g)-f_M(0)
=
\frac{1}{4!}\int_0^{g^4}
\langle\phi^4(x)\rangle_{\lambda}\,d\lambda.
\]

The current implementation constructs a cubic spline through the simulated values and integrates the spline analytically. The statistical uncertainty is propagated from the blocked-jackknife errors of \(\langle\phi^4\rangle\) using the corresponding spline-integration weights. HMC runs at different coupling values are treated as statistically independent.
...
> Nikita A. Ignatyuk and Daniel Skliannyi, *Strong-Weak Coupling Duality in Scalar Lattice QFT with Application to Strong Coupling Expansions*, arXiv:2207.11503.

For the current reproducible code snapshot, see the GitHub release
[`v1.1`](https://github.com/inochka/lattice-phi4/releases/tag/v1.1).