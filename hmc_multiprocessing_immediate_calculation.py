...
def compute_cfgs(params):
    ...
    cfgs = np.array(cfgs)
    corrs, errors = get_corr_func_mom(cfgs, get_momenta_grid(M, d)).T

    # TODO: как-то правильно скоммуницироваться между процессами, то ли сделать процесс-оркестратор, то ли лок навесить
    #  какой
